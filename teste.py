...
for _ in range(iteracoes):
    ...
    for f in fluxos_pos:
        ...
        fluxo_sort = np.random.normal(
            loc=f,
            scale=abs(f) * volatilidade_fluxos
        )

        fluxos_simulados.append(fluxo_sort)

    # Simulação da TMA
    tma_simulada = np.random.normal(
        loc=tma_base,
        scale=tma_base * volatilidade_tma
    )

    # Evita TMA negativa extrema
    tma_simulada = max(tma_simulada, -0.99)

    vpl_sim = calcular_vpl(fluxos_simulados, tma_simulada)
...
ax_mc.axvline(0, linestyle="--", label="VPL = 0")

ax_mc.axvline(media, linestyle=":", label="VPL médio")

ax_mc.set_title("Distribuição do VPL - Monte Carlo")
